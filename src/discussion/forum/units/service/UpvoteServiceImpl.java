...
public class UpvoteServiceImpl implements UpvoteService {
	...
	private Upvote getUpvote(User user, Question question, Reply reply) {
		...
		for (Upvote upvote : upvotes) {
			...
			*/
			if (upvote.getUser() == user) { // if upvoted user is the current user
				// if upvote.getReply() == reply is true, reply is already upvoted by the user.
				// similarly if upvote.getQuestion == question then the question is already upvoted by the user.
				if ((question == null && upvote.getReply() == reply) || (reply == null && upvote.getQuestion() == question)) {
					return upvote;
				}
				// if condition not satisfied returns null ie, question or reply is not upvoted and can be upvoted.
			}
		}
		return null;
	}
